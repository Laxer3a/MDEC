...
void SPU::step(CDRom* cdrom, Sample& audioOutLeft, Sample& audioOutRight) {
    ...
    if (control.cdEnable) {
		cdLeft  = cdrom->popLeft ();
		cdRight = cdrom->popRight();
...
//	printf("SPU WRITE %x (%i) (%i)\n",address, totalCount, writeCount);
	totalCount++;
...
*/
    ...
    if (address >= 0x1f801d88 && address <= 0x1f801d8b) {  // Voices Key On
        FOR_EACH_VOICE(address - 0x1f801d88, [&](int v, bool bit) {
            if (control.spuEnable && bit) voices[v].keyOn(currCycle);
//          if (bit && verbose) fmt::print("[SPU] W Voice {:2d}, KeyOn\n", v + 1);
    ...
    if (address >= 0x1f801d8c && address <= 0x1f801d8f) {  // Voices Key Off
        FOR_EACH_VOICE(address - 0x1f801d8c, [&](int v, bool bit) {
            if (control.spuEnable && bit) voices[v].keyOff(currCycle);
...
void CDRom::pushLeft (Sample p) {
	int nextWrite = writeLeft + 1;
	if (nextWrite == 2048) { nextWrite = 0; }

	// Accept to write only if not full.
	if (nextWrite != readLeft) {
		leftFIFO[writeLeft] = p;
		writeLeft = nextWrite;
	}
}

void CDRom::pushRight(Sample p) {
	int nextWrite = writeRight + 1;
	if (nextWrite == 2048) { nextWrite = 0; }

	// Accept to write only if not full.
	if (nextWrite != readRight) {
		rightFIFO[writeRight] = p;
		writeRight = nextWrite;
	}
}


Sample CDRom::popLeft () {
	Sample s = 0; // Do not return 0 when no value but return last value always...
		   s = leftFIFO[readLeft];
	if (readLeft != writeLeft) {
		readLeft++;
		if (readLeft == 2048) { readLeft = 0; }
	}
	return s;
}

Sample CDRom::popRight() {
	Sample s = 0; // Do not return 0 when no value but return last value always...
		   s = rightFIFO[readRight];
	if (readRight != writeRight) {
		readRight++;
		if (readRight == 2048) { readRight = 0; }
	}
	return s;
}
