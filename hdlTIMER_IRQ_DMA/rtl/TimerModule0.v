...
(
	...
	reg 			sys_IrqWhenTarget;	// Bit 4
	reg				sys_IrqWhenFull;	// Bit 5
	reg				sys_IrqRepeat;		// Bit 6
	reg				sys_IrqFlipMode;
	reg  [1:0]		sys_srcClockSel;
	reg				reachedTarget;
	reg				reachedFull;


	// NOTE THAT TIMER0 USES HBL SIG TO SYSCLOCK DOMAIN VERSION.
	// ======================================================
	// Transition from 0->1 for signal xBL
	reg  transitionXBL;									// NOT USED BY TIMER 0
	always @(posedge sysClk) begin
		transitionXBL       = i_xBL;
	end
	// ------------------------------------------------------
	wire sys_xBLTrans       = i_xBL       & !transitionXBL;
	// ======================================================

																						// ======================================================
																						// Transition from 0->1 for signal xBL
																						reg  transitionSecondSrc;									// NOT USED BY TIMER 0
																						always @(posedge sysClk) begin
	...
	wire xxx_isFull 	= (xxx_counter ==   16'hFFFF);
	wire xxx_isTarget	= (xxx_counter == sys_target);
	
	// SPECIAL CONDITION : In sys_mode 3 for type 1 -> Switch to freeRun to xBL transition to 1.
	wire sys_setFreeRun  = (TYPE != 2)  && (sys_mode == 2'd3) && sys_xBLTrans;
	
	// Depending on clock type, source clock selection is different.
	// (Name as no sys or xxx name because unused in TIMER 0)
	wire useExtClock = ((TYPE != 2) && (sys_srcClockSel[0]))  // 0 or 2 = System Clock,  1 or 3 = Hblank           (Sync VBlank)
	                || ((TYPE == 2) && (sys_srcClockSel[1])); // 0 or 1 = System Clock,  2 or 3 = System Clock/8   (No Sync)

	// --------------------------------------------------------------------
	// Used only for timer 1 and 2, 
	// in case of Timer 0, don't use logic to select clock !!!
	// 
	...
	begin
		if (TYPE == 0) begin
			...
			freeRunIncr = (!useExtClock) | (useExtClock & secondClkTrans);
		end
	end
	// --------------------------------------------------------------------
	wire timer0ClkMux = (sys_srcClockSel[0] ? pixClk : sysClk);
	wire xxx_clk      = (TYPE != 0) ? sysClk : timer0ClkMux;
	// --------------------------------------------------------------------
	
	// Reset logic for Register bit [3]
	wire resetBase	 = (sys_resetType & xxx_isTarget) | (!sys_resetType & xxx_isFull);
	...
	begin
		if (sys_freeRun) begin
			...
			if (TYPE != 2) begin
				...
				2'd3: begin
					incr	= sys_setFreeRun;			// Pause until xBlank == 1. Allow to increment right away.
					reset	= resetBase;
				end
				endcase
			end
	...
	// ------------------------------------------------------------------------
	wire basePulse	= ((sys_IrqWhenTarget & xxx_isTarget) | (sys_IrqWhenFull & xxx_isFull));
	reg fired;
	// If IRQ condition met -> Fire.
	// If already fired, do authorize repeat ?
	// If not     fired, always true.
	wire setIRQ		= ((!fired & !sys_IrqRepeat) | sys_IrqRepeat) & basePulse;
	reg  prevIRQ;
...
//	wire    exportIRQ = sys_IrqFlipMode       ? (transitionIRQ ? (!outIRQ) : outIRQ) : (transitionIRQ ? 1'b0 : outIRQ);
	// Equivalent : if transition, return next value, else current value.
	wire    exportIRQ = transitionIRQ ? (sys_IrqFlipMode ? (!outIRQ) :1'b0 ) : outIRQ; // Equivalent.
	assign	i_xxx_irqTimer = !exportIRQ;
	
	always @(posedge xxx_clk)
	begin
		if (i_nRst == 1'b0) begin
			xxx_counter		= 16'd0;
		end else begin
			...
			if (reset) begin
				...
				if (i_sys_CSTimer && i_sys_write && (i_sys_regID==2'd0))  // PB HERE !!!!
				begin
	...
	begin
		if (i_nRst == 1'b0) begin
			...
			sys_mode			= 2'd0;
			sys_resetType		= 1'b0;
			sys_IrqWhenTarget	= 1'b0;
			sys_IrqWhenFull		= 1'b0;
			sys_IrqRepeat			= 1'b0;
			sys_IrqFlipMode				= 1'b0;
			sys_srcClockSel			= 2'd0;
			reachedTarget		= 1'b0;
			reachedFull			= 1'b0;
			fired				= 1'b0;
		end else begin
			// reached flag can be overridden by CPU READ or WRITE.
			if (xxx_isTarget) begin
				reachedTarget	= 1'b1;
			end
			if (xxx_isFull) begin
				reachedFull		= 1'b1;
			...
			if (i_sys_CSTimer) begin
				if (i_sys_write) begin
					...
					begin
						...
						sys_IrqWhenTarget	= i_sys_valueW[4];
						sys_IrqWhenFull		= i_sys_valueW[5];
						sys_IrqRepeat		= i_sys_valueW[6];
						sys_IrqFlipMode				= i_sys_valueW[7];
						sys_srcClockSel			= i_sys_valueW[9:8];
// Validated by test.
//						reachedTarget		= 1'b0;
//						reachedFull			= 1'b0;
						fired				= 1'b0;
	...
	//   [CPU READ SIDE]
	// ----------------------------------------------------------
	wire [15:0] modeR = { 3'b000,reachedFull,reachedTarget,exportIRQ,sys_srcClockSel,sys_IrqFlipMode,sys_IrqRepeat,sys_IrqWhenFull,sys_IrqWhenTarget,sys_resetType, sys_mode,sys_freeRun };
