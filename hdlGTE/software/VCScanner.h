...
public:
	...
	bool init		(int maxmemberCount, bool forceDefaultDump);
	void shutdown	();
...
private:
	...
	bool        compareOldNew() {
		bool res = memcmp(currValuePointer, oldValuePointer, sizeByte); 
		res |= first;
		first = false;
		return res;
	}
...
bool VCScanner::init(int maxmemberCount, bool forceDefaultDump_) {
	if (members == NULL) {
		...
		members[0].first = true;

		forceDefaultDump = forceDefaultDump_;
